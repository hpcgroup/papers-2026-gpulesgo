...
($3072 \times 384 \times 400$), on perlmutter compute node each 
with 64-core AMD EPYC 7763 (Milan) CPUs.
As shown in Figure~\ref{fig:ceiling}, increasing the CPU count from 
20 to 40 cores yields virtually no performance improvement. 
Even when scaling out across 4 nodes to 200 CPU cores, 
where the 1D slab decomposition reaches its structural 
limit of two vertical planes per rank, the execution time plateaus at 5.09\,s 
per step. At this scaling ceiling, accumulating converged turbulence statistics

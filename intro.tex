...
different kinds of simulations including wind-farm~\cite{calaf2010,stevens2014precursor,
stevens2016spacing,shapiro2019fad,narasimhan2022veer,zhu2025jhtdbwind,xiao2025diurnal}, rough-surface~\cite{anderson2010,
anderson2011,anderson2012}, tree canopies~\cite{graham2012}, urban flow and 
dispersion~\cite{tseng2006}, and wall-bounded turbulence~\cite{porteagel2000,bouzeid2005,
yang2015iwm,bretheim2015}. 
% Most recently, it has produced public wind-farm LES databases such as
% JHTDB-wind~\cite{zhu2025jhtdbwind,xiao2025diurnal}.
Figure~\ref{fig:wf60-velocity} illustrates the interacting turbulent wakes in
the 60-turbine JHTDB-wind configuration, which serves as our production
benchmark. LESGO's numerical formulation combines Fourier
...
encounter severe limitations here. (1)~Array mirroring keeps a device
copy of host array, so kernels read device-resident state and data
returns to the host only where host code still consumes
it~\cite{esclapez2025dales,giorgetta2022}. This determines where the data 
lives but leaves open how to serve the remaining host consumers efficiently, 
since each full-field transfer back to the host stalls the GPU. Mirroring every
array also consumes device memory, so that LESGO needs more GPUs for the
same problem size.
...
Thus, achieving high performance for LESGO requires a fine-grained data
residency strategy tailored to its distinct host--device coupling points.
...
production-scale wind-farm LES within practical reach. We begin by measuring
two placement strategies on LESGO, array mirroring and
CUDA managed memory, and trace the bottleneck in both to host--device
coupling points. Guided by this analysis, we analyze all coupling points across the solver,
classify them by host consumer read footprints, and apply four treatments: 
\emph{gate} skips a transfer whenever its
conditional or infrequent consumer does not execute; \emph{relocate} ports
the consumer to the GPU and removes the transfer altogether; \emph{slice}
shrinks a transfer to the subset of the field the host touches; and
\emph{overlap} hides an irreducible transfer behind concurrent GPU execution.
Beyond these data-residency treatments, we apply three further optimizations
to the remaining structural bottlenecks. We pipeline
...
and evaluate it on NVIDIA A100 GPUs. 
...
four distinct treatments (gate, relocate, slice, and overlap).
...
wind farm, achieving a 40$\times$ speedup over the fastest CPU baseline at
equal node count and up to 90$\times$ on 64 GPUs.

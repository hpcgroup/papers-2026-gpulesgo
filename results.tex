...
\;+ chunk pipelining (\S\ref{sec:opt-tridag})
                                       & 0.192 & 33.8$\times$ \\
\;+ batching (\S\ref{sec:opt-atm})
                                       & 0.186 & 27.5$\times$ \\
\;+ heterogeneous overlap (\S\ref{sec:opt-overlap})
                                       & \textbf{0.162} & \textbf{40$\times$} \\
\midrule
CPU, 4 nodes, with LASD cadence       & 8.18 & 1.0$\times$ \\
...
progression. These results demonstrate that our fine-grained data management
...
at 0.084\,s per time
step, achieving a throughput of 18.8 billion cell updates per second.
...
GPUs, the combined derivative and SGS stages fall from 61\,ms to
19\,ms and the turbine stage from 8\,ms to 2\,ms, while the pressure solver exhibits
...
approximately 50\,ms, which already accounts for most of the 72\,ms
step at 64 GPUs. This overhead primarily
...
batching thousands of small launches (Section~\ref{sec:opt-atm}),
and the pipeline latency floor represents
...
kernels reach 48--82\% of peak memory bandwidth. The residual gap on

...
backends produce consistent LESGO results. Figure~\ref{fig:lesgo-cpu-gpu-field}
...
$\Delta t=0.0014$, and 1,000 steps; the CPU run uses 48 MPI ranks and the
GPU run two A100 GPUs. At the common physical time
...
velocity difference is $9.00\times10^{-14}$. The GPU-minus-CPU difference
(Fig.~\ref{fig:lesgo-cpu-gpu-field}c) follows the wake structure but stays
at roundoff scale, and the hub-height centerline profiles
(Fig.~\ref{fig:lesgo-cpu-gpu}) are indistinguishable, confirming that the
two backends are consistent.

\begin{figure*}[t]
    \centering
    \includegraphics[width=1\linewidth]{figs/fig_cpu_gpu_lesgo_fields.pdf}
    \caption{Matched CPU and GPU runs of one NREL 5-MW turbine after
    1,000 steps. (a),~(b)~Normalized axial velocity $u/U_\infty$ on the
    vertical rotor-center plane, same color scale. (c)~GPU-minus-CPU
    difference, which stays at roundoff scale ($\sim\!10^{-14}$).}
    \label{fig:lesgo-cpu-gpu-field}
\end{figure*}

\begin{figure}[h]
    \centering
    \includegraphics[width=1\linewidth]{figs/fig_cpu_gpu_lesgo_centerline.pdf}
    \caption{Hub-height centerline profiles of $u/U_\infty$ from the CPU
    and GPU runs of Fig.~\ref{fig:lesgo-cpu-gpu-field}. The two curves
    coincide.}
